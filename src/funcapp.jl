...
x_new = linspace(lb,ub,n_new)
	 ...
	 p = Any[]
	 true_approx=plot(1:n_new, y_true, label="True f(x)", title="True vs Apprx_Cheb")
	 plot!(y_new, label="Approximated with Cheby")
	 push!(p,true_approx)
	 push!(p, plot(1:n_new, err, label="Deviation from true", title="Chebyshev error"))
...
function runall()
		...
		println("Runge's phenomenon is a problem of oscillation at the edges of an interval.")
		println("Having higher degrees does not always improve approximation")
		println("Question 4b")
		display(q4b())
		println("Couldn't handle knot multiplicity with ApproXD, and other issues too")
		println("Used CompEcon instead for question 5")
		display(q5())
		println("3 zero knots give a kink")
	end
end
